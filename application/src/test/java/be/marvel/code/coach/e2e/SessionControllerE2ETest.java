...
import be.marvel.code.coach.service.service.SessionService;
...
//        personService.becomeCoach(topicList, savedCoach.getId());
//
...
//
//        //THEN
//        Assertions.assertThat(actualResult.getCoachee().getId()).isEqualTo(savedCoachee.getId());
...
//    }
    ...
    private CreateSessionDto createSessionFactory(UUID coach, UUID coachee){
        ...
        Person coachPerson = personService.getById(coach);

        return new CreateSessionDto()
                .setCoacheeId(coachee)
                .setTopic(coachPerson.getTopics().get(0).getId())
                .setDate(strDate)
                .setTime(strTime)
                .setLocation("Online")
                .setRemarks("Remarks");
